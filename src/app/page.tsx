...
export default async function HomePage({
  ...
  const { page, state, ages, dateFrom, dateTo, sanction, search } = await searchParams

  const selectedState = state ?? 'MO'
  const selectedAges = ages ? ages.split(',').filter(Boolean) : []
  const selectedDateFrom = dateFrom ?? ''
  const selectedDateTo = dateTo ?? ''
  const selectedSanction = sanction ?? 'all'
  const selectedSearch = search ?? ''
  const currentPage = Math.max(1, parseInt(page ?? '1', 10))

  const allEvents = await fetchAllEvents()

  const availableStates = [...new Set(allEvents.map((e) => e.state).filter(Boolean))] as string[]
  const availableSanctions = [...new Set(allEvents.map((e) => e.sanction))]

  const availableAges = [
    ...new Set(allEvents.flatMap((e) => e.ageGroups.map((ag) => ag.label))),
  ...
  let filtered = selectedState === 'all'
    ? sorted
    : sorted.filter((e) => e.state === selectedState)
